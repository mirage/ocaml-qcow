...
let complete_read_bytes fd buf =
  ...
  let rec loop buf buf_offset len =
    let* read_bytes_nr = read_bytes fd buf ~buf_offset ~len in
    Log.debug (fun f -> f "read %d bytes, len left %d\n" read_bytes_nr len) ;
    if read_bytes_nr = len then
      return ()
    else if read_bytes_nr = 0 then
      fail End_of_file
    else
      loop buf (buf_offset + read_bytes_nr) (len - read_bytes_nr)
  in
